...
class EMAILWrapper {
  ...
  private function __construct() {
    ...
    if (class_exists('PHPMailerLite')) {
      $this->mail = new PHPMailerLite(); // needs http://sourceforge.net/projects/phpmailer/
      $this->mail->From = BOT_EMAIL;
      $this->mail->Sender = BOT_EMAIL;
      $this->mail->WordWrap = 80;
      $this->mail->SingleTo = true;
      $this->mail->action_function = array($this, 'callBack');
      if (EMAIL_IS_SENDMAIL) $this->mail->IsSendmail();
  ...
  // Provide a public callback
  public function callBack($isSent, $to, $cc, $bcc, $subject, $body) { 
    $to  = self::cleanEmails($to);
    $line = trim(date("[d/m @ H:i:s]") . "EMAIL: " . (($isSent) ? 'success' : $this->mail->ErrorInfo) . "\tTo: "  . $to . "\tSubject: "  . $subject) . "\n";
    error_log($line, 3, LOG_FILE);
    return true;    
  }

  // send email
  public function sendEMAIL($receiver, $message, $subject, $from, $confirm = false) {
    try{
      $this->mail->ClearAllRecipients();
      $this->mail->ClearAttachments();
      $this->mail->ClearCustomHeaders();
      $this->mail->ClearErrorMessage();
      $this->mail->AddAddress($receiver);
      $this->mail->Body = $message;
      $this->mail->Subject  = $subject;
      $this->mail->MessageID = uniqid();
      $this->mail->FromName = $from;
      if ($confirm) $this->mail->ConfirmReadingTo = BOT_EMAIL;
      else $this->mail->ConfirmReadingTo = '';
      $this->mail->Send();
      
      return array('code'   => -1,
                   'text'   => "Email versendet.",
                   'id'     => $this->mail->MessageID,
                   'error'  => false);
    }
    catch (phpmailerException $e){
      $line = trim(date("[d/m @ H:i:s]") . "EMAIL phpmailerException: " . $e->errorMessage()) . "\n";  
      error_log($line, 3, EMAIL_LOG_FILE);
      return array('text'  => $e->getMessage(),
                   'code'  => $e->getCode(),
                   'error' => true);
    }
    catch (Exception $e){
      $line = trim(date("[d/m @ H:i:s]") . "EMAIL Exception: " . $e->getMessage()) . "\n";  
      error_log($line, 3, EMAIL_LOG_FILE);
      return array('text'  => $e->getMessage(),
                   'code'  => $e->getCode(),
                   'error' => true);
  ...
  private static function cleanEmails($str) {
 ...
 */
  ...
  */
  ...
  protected function doCallback($isSent,$to,$cc,$bcc,$subject,$body) {
    if (!empty($this->action_function) && ((is_array($this->action_function) && method_exists($this->action_function[0], $this->action_function[1])) || function_exists($this->action_function))) {
...
// get instance of EMAIL Gateway
$mail = EMAILWrapper::GetInstance();
?>