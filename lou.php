    ...
    public function getCached($endpoint, $data = array(), $noerror = false, $expire = 180) {
      ...
      if ($cache_test || $redis->TTL($key) === -1) {
        ...
        if (!$this->error) {
          ...
          if ($cache_test && $serialized) {
            $unserialized = unserialize($serialized);
            $this->debug("CacheTest: ".REDIS_NAMESPACE.$key);
            if ($this->debug) print_r(array_diff_assoc($unserialized, $decode));
          }
