<?php
/**
 * Receives responses of the SMS-Type "standard"
 *
...
$data = array('msg_id'    => $_POST['messageId'],
              'number'    => $_POST['responseSender'],
              'message'   => $_POST['responseMessage'],
              'timestamp' => $_POST['responseTimestamp']);
    ...
    if ($redis->EXISTS("sms:send:{$data['msg_id']}")) {
      ...
      if (is_array($send)) {
        ...
        $redis->HMSET("sms:send:{$data['msg_id']}", array(
                      'status'    => SMS_STATUS_ANSWERED));
        $redis->HMSET("sms:receive:{$data['msg_id']}", array(
                      'returnto'  => $send['returnto'],
